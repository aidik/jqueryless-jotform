var Prototype = {
    ...
    BrowserFeatures: {
        ...
        ElementExtensions: (function() { var constructor = window.Element || window.HTMLElement; return !!(constructor && constructor.prototype); })(),
...
(function() {
    ...
    function Str(key, holder, stack) {
        ...
        switch (type) {
            ...
            case 'object':
                for (var i = 0, length = stack.length; i < length; i++) { if (stack[i] === value) { throw new TypeError(); } }
                stack.push(value);
                var partial = [];
    ...
    function keys(object) {
        ...
        var results = [];
        for (var property in object) { if (object.hasOwnProperty(property)) { results.push(property); } }
        return results;
    }
...
Object.extend(Function.prototype, (function() {
    ...
    function bindAsEventListener(context) {
        var __method = this,
            args = slice.call(arguments, 1);
        return function(event) { var a = update([event || window.event], args); return __method.apply(context, a); }
    }
    ...
    function curry() {
        ...
        return function() { var a = merge(args, arguments); return __method.apply(this, a); }
    }
    ...
    onTimerEvent: function() {
        if (!this.currentlyExecuting) {
            try {
                ...
                this.execute();
                this.currentlyExecuting = false;
            } catch (e) { this.currentlyExecuting = false; throw e; }
        }
    }
});
    ...
    function toQueryParams(separator) {
        ...
        return match[1].split(separator || '&').inject({}, function(hash, pair) {
            if ((pair = pair.split('='))[0]) {
                ...
                if (value != undefined) try { value = decodeURIComponent(value) } catch (e) { value = unescape(value) }
                if (key in hash) {
...
var Enumerable = (function() {
    function each(iterator, context) {
        var index = 0;
        try { this._each(function(value) { iterator.call(context, value, index++); }); } catch (e) { if (e != $break) throw e; }
        return this;
    }
...
function $A(iterable) {
    if (typeof window.Set !== 'undefined' && iterable instanceof window.Set) {
        var arr = [];
        iterable.forEach(function(x) { arr.push(x); });
        return arr;
    }
    if (!iterable) return [];
...
(function() {
    ...
    function each(iterator, context) { for (var i = 0, length = this.length >>> 0; i < length; i++) { if (i in this) iterator.call(context, this[i], i, this); } }
    if (!_each) _each = each;
    ...
    function indexOf(item, i) {
        ...
        i = Number(i);
        if (isNaN(i)) { i = 0; } else if (i !== 0 && isFinite(i)) { i = (i > 0 ? 1 : -1) * Math.floor(Math.abs(i)); }
    ...
    function lastIndexOf(item, i) {
        ...
        var array = Object(this),
            length = array.length >>> 0;
        if (length === 0) return -1;
        if (!Object.isUndefined(i)) { i = Number(i); if (isNaN(i)) { i = 0; } else if (i !== 0 && isFinite(i)) { i = (i > 0 ? 1 : -1) * Math.floor(Math.abs(i)); } } else { i = length; }
    ...
    function wrapNative(method) {
        return function() {
            if (arguments.length === 0) { return method.call(this, Prototype.K); } else if (arguments[0] === undefined) {
    ...
    function filter(iterator) {
        ...
        var results = [],
            context = arguments[1],
            value;
        for (var i = 0, length = object.length >>> 0; i < length; i++) { if (i in object) { value = object[i]; if (iterator.call(context, value, i, object)) { results.push(value); } } }
        return results;
    }
    ...
    function some(iterator) {
        ...
        var object = Object(this);
        for (var i = 0, length = object.length >>> 0; i < length; i++) { if (i in object && iterator.call(context, object[i], i, object)) { return true; } }
        return false;
    }
    ...
    function every(iterator) {
        ...
        var context = arguments[1];
        var object = Object(this);
        for (var i = 0, length = object.length >>> 0; i < length; i++) { if (i in object && !iterator.call(context, object[i], i, object)) { return false; } }
        return true;
    }
    if (arrayProto.every) {
        var unsafeEvery = wrapNative(Array.prototype.every);
        var safeEvery = function() { if (arguments.length === 0) return unsafeEvery(Prototype.K); return unsafeEvery.apply(this, arguments); }
        every = safeEvery;
    }
    ...
    var CONCAT_ARGUMENTS_BUGGY = (function() { return [].concat(arguments)[0][0] !== 1; })(1, 2);
...
Ajax.Response = Class.create({
    ...
    _getHeaderJSON: function() {
        ...
        try { return json.evalJSON(this.request.options.sanitizeJSON || !this.request.isSameOrigin()); } catch (e) { this.request.dispatchException(e); }
    },
    _getResponseJSON: function() {
        ...
        if (!options.evalJSON || (options.evalJSON != 'force' && !(this.getHeader('Content-type') || '').include('application/json')) || this.responseText.blank())
            return null;
        try { return this.responseText.evalJSON(options.sanitizeJSON || !this.request.isSameOrigin()); } catch (e) { this.request.dispatchException(e); }
    }
});
...
(function(global) {
    ...
    var HAS_EXTENDED_CREATE_ELEMENT_SYNTAX = (function() {
        try { var el = document.createElement('<input name="x">'); return el.tagName.toLowerCase() === 'input' && el.name === 'x'; } catch (err) { return false; }
    })();
...
Element.Methods = {
    ...
    update: (function() {
        ...
        function update(element, content) {
            ...
            if (tagName === 'SCRIPT' && SCRIPT_ELEMENT_REJECTS_TEXTNODE_APPENDING) { element.text = content; return element; }
    ...
    ancestors: function(element) { return Element.recursivelyCollect(element, 'parentNode'); },
    ...
    previousSiblings: function(element, maximumLength) { return Element.recursivelyCollect(element, 'previousSibling'); },
    nextSiblings: function(element) { return Element.recursivelyCollect(element, 'nextSibling'); },
    siblings: function(element) { element = $(element); return Element.previousSiblings(element).reverse().concat(Element.nextSiblings(element)); },
    ...
    up: function(element, expression, index) { element = $(element); if (arguments.length == 1) return $(element.parentNode); var ancestors = Element.ancestors(element); return Object.isNumber(expression) ? ancestors[expression] : Prototype.Selector.find(ancestors, expression, index); },
    down: function(element, expression, index) { element = $(element); if (arguments.length == 1) return Element.firstDescendant(element); return Object.isNumber(expression) ? Element.descendants(element)[expression] : Element.select(element, expression)[index || 0]; },
    previous: function(element, expression, index) { element = $(element); if (Object.isNumber(expression)) index = expression, expression = false; if (!Object.isNumber(index)) index = 0; if (expression) { return Prototype.Selector.find(element.previousSiblings(), expression, index); } else { return element.recursivelyCollect("previousSibling", index + 1)[index]; } },
    next: function(element, expression, index) { element = $(element); if (Object.isNumber(expression)) index = expression, expression = false; if (!Object.isNumber(index)) index = 0; if (expression) { return Prototype.Selector.find(element.nextSiblings(), expression, index); } else { var maximumLength = Object.isNumber(index) ? index + 1 : 1; return element.recursivelyCollect("nextSibling", index + 1)[index]; } },
    select: function(element) { element = $(element); var expressions = Array.prototype.slice.call(arguments, 1).join(', '); return Prototype.Selector.select(expressions, element); },
    adjacent: function(element) { element = $(element); var expressions = Array.prototype.slice.call(arguments, 1).join(', '); return Prototype.Selector.select(expressions, element.parentNode).without(element); },
    ...
    getHeight: function(element) { return Element.getDimensions(element).height; },
    getWidth: function(element) { return Element.getDimensions(element).width; },
    classNames: function(element) { return new Element.ClassNames(element); },
    hasClassName: function(element, className) { if (!(element = $(element))) return; var elementClassName = element.className; return (elementClassName && elementClassName.length > 0 && (elementClassName == className || new RegExp("(^|\\s)" + className + "(\\s|$)").test(elementClassName))); },
    ...
    toggleClassName: function(element, className) { if (!(element = $(element))) return; return Element[Element.hasClassName(element, className) ? 'removeClassName' : 'addClassName'](element, className); },
    ...
    setStyle: function(element, styles) {
        ...
        var elementStyle = element.style,
            match;
        if (Object.isString(styles)) { element.style.cssText += ';' + styles; return styles.include('opacity') ? element.setOpacity(styles.match(/opacity:\s*(\d?\.?\d*)/)[1]) : element; }
...
if (Prototype.Browser.Opera) {
        ...
        switch (style) {
            ...
            case 'width':
                ...
                if (dim !== element['offset' + style.capitalize()])
                    return dim + 'px';
                var properties;
                if (style === 'height') { properties = ['border-top-width', 'padding-top', 'padding-bottom', 'border-bottom-width']; } else { properties = ['border-left-width', 'padding-left', 'padding-right', 'border-right-width']; }
    ...
    Element._attributeTranslations = (function() {
        ...
        if (el.className !== 'x') { el.setAttribute('class', 'x'); if (el.className === 'x') { classProp = 'class'; } }
        el = null;
        el = document.createElement('label');
        el.setAttribute(forProp, 'x');
        if (el.htmlFor !== 'x') { el.setAttribute('htmlFor', 'x'); if (el.htmlFor === 'x') { forProp = 'htmlFor'; } }
        el = null;
        return {
            read: {
                names: { 'class': classProp, 'className': classProp, 'for': forProp, 'htmlFor': forProp },
                values: {
                    _getAttr: function(element, attribute) { return element.getAttribute(attribute); },
                    _getAttr2: function(element, attribute) { return element.getAttribute(attribute, 2); },
                    _getAttrNode: function(element, attribute) { var node = element.getAttributeNode(attribute); return node ? node.value : ""; },
                    _getEv: (function() {
                        var el = document.createElement('div'),
                            f;
    ...
    Element._attributeTranslations.write = { names: Object.extend({ cellpadding: 'cellPadding', cellspacing: 'cellSpacing' }, Element._attributeTranslations.read.names), values: { checked: function(element, value) { element.checked = !!value; }, style: function(element, value) { element.style.cssText = value ? value : ''; } } };
...
if ('outerHTML' in document.documentElement) {
    Element.Methods.replace = function(element, content) {
        element = $(element);
        if (content && content.toElement) content = content.toElement();
        if (Object.isElement(content)) { element.parentNode.replaceChild(content, element); return element; }
        content = Object.toHTML(content);
...
if (document.documentElement.hasAttribute) { Element.hasAttribute = function(element, attribute) { return element.hasAttribute(attribute); }; } else { Element.hasAttribute = Element.Methods.Simulated.hasAttribute; }
...
Element.addMethods({
    ...
    store: function(element, key, value) {
        if (!(element = $(element))) return;
        if (arguments.length === 2) { Element.getStorage(element).update(key); } else { Element.getStorage(element).set(key, value); }
        return element;
    },
...
(function() {
    ...
    function getPixelValue(value, property, context) {
        ...
        if (element && isPercentage) {
            ...
            if (context === document.viewport) { if (isHorizontal) { whole = document.viewport.getWidth(); } else if (isVertical) { whole = document.viewport.getHeight(); } } else { if (isHorizontal) { whole = $(context).measure('width'); } else if (isVertical) { whole = $(context).measure('height'); } }
    ...
    Element.Layout = Class.create(Hash, {
        initialize: function($super, element, preCompute) {
            ...
            Element.Layout.PROPERTIES.each(function(property) { this._set(property, null); }, this);
            if (preCompute) {
        ...
        get: function($super, property) { var value = $super(property); return value === null ? this._compute(property) : value; },
        _begin: function() {
            if (this._prepared) return;
            var element = this.element;
            if (isDisplayed(element)) { this._prepared = true; return; }
            ...
            var newWidth;
            if (width && (positionedWidth === width)) { newWidth = getPixelValue(element, 'width', context); } else if (position === 'absolute' || position === 'fixed') { newWidth = getPixelValue(element, 'width', context); } else {
                var parent = element.parentNode,
        ...
        toObject: function() {
            ...
            var obj = {};
            keys.each(function(key) { if (!Element.Layout.PROPERTIES.include(key)) return; var value = this.get(key); if (value != null) obj[key] = value; }, this);
            return obj;
        },
        toHash: function() { var obj = this.toObject.apply(this, arguments); return new Hash(obj); },
        toCSS: function() {
            var args = $A(arguments);
            var keys = (args.length === 0) ? Element.Layout.PROPERTIES : args.join(' ').split(' ');
            var css = {};
            keys.each(function(key) { if (!Element.Layout.PROPERTIES.include(key)) return; if (Element.Layout.COMPOSITE_PROPERTIES.include(key)) return; var value = this.get(key); if (value != null) css[cssNameFor(key)] = value + 'px'; }, this);
            return css;
        },
    ...
    Object.extend(Element.Layout, {
        ...
        COMPUTATIONS: {
            ...
            'border-box-height': function(element) { if (!this._preComputing) this._begin(); var height = element.offsetHeight; if (!this._preComputing) this._end(); return height; },
            'cumulative-left': function(element) { return element.cumulativeOffset().left; },
            'cumulative-top': function(element) { return element.cumulativeOffset().top; },
            'border-box-width': function(element) { if (!this._preComputing) this._begin(); var width = element.offsetWidth; if (!this._preComputing) this._end(); return width; },
            ...
            'top': function(element) { var offset = element.positionedOffset(); return offset.top; },
            ...
            'left': function(element) { var offset = element.positionedOffset(); return offset.left; },
    ...
    function positionedOffset(element) {
        ...
        do {
            ...
            valueL += element.offsetLeft || 0;
            element = element.offsetParent;
            if (element) { if (isBody(element)) break; var p = Element.getStyle(element, 'position'); if (p !== 'static') break; }
        } while (element);
...
window.$$ = function() { var expression = $A(arguments).join(', '); return Prototype.Selector.select(expression, document); };
    ...
    var Expr = Sizzle.selectors = {
        ...
        filter: {
            PSEUDO: function(elem, match, i, array) {
                ...
                if (filter) { return filter(elem, i, match, array); } else if (name === "contains") { return (elem.textContent || elem.innerText || "").indexOf(match[3]) >= 0; } else if (name === "not") {
                    var not = match[3];
...
var Form = {
    ...
    serializeElements: function(elements, options) {
        ...
        if (options.hash) {
            ...
            initial = '';
            accumulator = function(result, key, value) { return result + (result ? '&' : '') + encodeURIComponent(key) + '=' + encodeURIComponent(value); }
        }
...
Form.Methods = {
    serialize: function(form, options) { return Form.serializeElements(Form.getElements(form), options); },
    getElements: function(form) {
        var elements = $(form).getElementsByTagName('*'),
            element, arr = [],
            serializers = Form.Element.Serializers;
        for (var i = 0; element = elements[i]; i++) { arr.push(element); }
    ...
    focusFirstElement: function(form) { form = $(form); var element = form.findFirstElement(); if (element) element.activate(); return form; },
...
Form.Element.Methods = {
    ...
    disable: function(element) {
        ...
        if (element.type === 'file') {
            ...
            deleteUploadBtns.each(function(btn) { btn.style.visibility = 'hidden'; });
        }
        return element;
    },
    enable: function(element) {
        ...
        if (element.type === 'file') {
            var deleteUploadBtns = $$('#' + element.closest('li').id + ' .qq-upload-delete');
            deleteUploadBtns.each(function(btn) { btn.style.visibility = ''; });
        }
        return element;
    }
};
...
Form.Element.Serializers = (function() {
    ...
    function selectMany(element) {
        ...
        for (var i = 0, values = []; i < length; i++) { var opt = element.options[i]; if (opt.selected) values.push(optionValue(opt)); }
        return values;
    }
...
(function() {
    ...
    if (window.attachEvent) { if (window.addEventListener) { isIELegacyEvent = function(event) { return !(event instanceof window.Event); }; } else { isIELegacyEvent = function(event) { return true; }; } }
    var _isButton;
    ...
    function pointerX(event) {
        ...
        if (('createTouch' in document) && event.touches) {
            if (event.touches[0]) { return event.touches[0].pageX; } else { return event.pageX; }
        }
    ...
    function pointerY(event) {
        ...
        if (('createTouch' in document) && event.touches) {
            if (event.touches[0]) { return event.touches[0].pageY; } else { return event.pageY; }
        }
    ...
    if (window.attachEvent) {
        ...
        var additionalMethods = { stopPropagation: function() { this.cancelBubble = true }, preventDefault: function() { this.returnValue = false }, inspect: function() { return '[object Event]' } };
    ...
    function _createResponder(element, eventName, handler) {
        ...
        try { respondersForEvent = registry.get(eventName); } catch (e) { return false; }
    ...
    if (!MOUSEENTER_MOUSELEAVE_EVENTS_SUPPORTED) { _getDOMEventName = function(eventName) { return (translations[eventName] || eventName); }; }
    ...
    function stopObserving(element, eventName, handler) {
        ...
        if (!eventName) {
            try {
                ...
                return element;
            } catch (err) { return element; }
        }
        var responders = registry.get(eventName);
        if (!responders) return element;
        if (!handler) { responders.each(function(r) { stopObserving(element, eventName, r.handler); }); return element; }
        var i = responders.length,
            responder;
        while (i--) { if (responders[i].handler === handler) { responder = responders[i]; break; } }
        if (!responder) return element;
        if (eventName.include(':')) {
    ...
    Event.Handler = Class.create({
        ...
        start: function() { Event.observe(this.element, this.eventName, this.handler); return this; },
        stop: function() { Event.stopObserving(this.element, this.eventName, this.handler); return this; },
        handleEvent: function(event) { var element = Event.findElement(event, this.selector); if (element) this.callback.call(this.element, event, element); }
    });
...
(function() {
    ...
    function pollDoScroll() {
        try { document.documentElement.doScroll('left'); } catch (e) { timer = pollDoScroll.p_defer(); return; }
        fireContentLoadedEvent();
    }
...
(function() {
    window.Selector = Class.create({ initialize: function(expression) { this.expression = expression.strip(); }, findElements: function(rootElement) { return Prototype.Selector.select(this.expression, rootElement); }, match: function(element) { return Prototype.Selector.match(element, this.expression); }, toString: function() { return this.expression; }, inspect: function() { return "#<Selector: " + this.expression + ">"; } });
    Object.extend(Selector, {
        matchElements: function(elements, expression) {
            var match = Prototype.Selector.match,
                results = [];
            for (var i = 0, length = elements.length; i < length; i++) { var element = elements[i]; if (match(element, expression)) { results.push(Element.extend(element)); } }
            return results;
        },
        findElement: function(elements, expression, index) {
            ...
            var matchIndex = 0,
                element;
            for (var i = 0, length = elements.length; i < length; i++) { element = elements[i]; if (Prototype.Selector.match(element, expression) && index === matchIndex++) { return Element.extend(element); } }
        },
...
if (window.console === undefined) {
    if (!window.console || !console.firebug) {
        (function(m, i) { window.console = {}; var e = function() {}; while (i--) { window.console[m[i]] = e; } })('log debug info warn error assert dir dirxml trace group groupEnd time timeEnd profile profileEnd count'.split(' '), 16);
    }
...
Protoplus = {
    ...
    Colors: {
        ...
        getPalette: function() {
            ...
            var i = 0;
            for (var r = 0; r < cr.length; r++) { for (var g = 0; g < cr.length; g++) { for (var b = 0; b < cr.length; b++) { generated[(i++) + "_"] = '#' + cr[r] + cr[g] + cr[b]; } } }
            return generated;
        },
        getRGBarray: function(color) {
            if (typeof color == "string") { if (color.indexOf("rgb") > -1) { color = color.replace(/rgb\(|\).*?$/g, "").split(/,\s*/, 3); } else { color = color.replace("#", ""); if (color.length == 3) { color = color.replace(/(.)/g, function(n) { return parseInt(n + n, 16) + ", "; }).replace(/,\s*$/, "").split(/,\s+/); } else { color = color.replace(/(..)/g, function(n) { return parseInt(n, 16) + ", "; }).replace(/,\s*$/, "").split(/,\s+/); } } }
        ...
        rgbToHex: function() {
            var ret = [];
            var ret2 = [];
            for (var i = 0; i < arguments.length; i++) { ret.push((arguments[i] < 16 ? "0" : "") + Math.round(arguments[i]).toString(16)); }
            return "#" + ret.join('').toUpperCase();
        },
        hexToRgb: function(str) {
            str = str.replace("#", "");
            var ret = [];
            if (str.length == 3) { str.replace(/(.)/g, function(str) { ret.push(parseInt(str + str, 16)); }); } else { str.replace(/(..)/g, function(str) { ret.push(parseInt(str, 16)); }); }
            return ret;
        },
        invert: function(hex) { var rgb = Protoplus.Colors.hexToRgb(hex); return Protoplus.Colors.rgbToHex(255 - rgb[0], 255 - rgb[1], 255 - rgb[2]); }
    },
    Profiler: {
        stimes: {},
        start: function(title) { Protoplus.Profiler.stimes[title] = (new Date()).getTime(); },
...
Object.extend(Object, {
    deepClone: function(obj) {
        ...
        for (var i in obj) { var node = obj[i]; if (typeof node == 'object') { if (Object.isArray(node)) { clone[i] = []; for (var j = 0; j < node.length; j++) { if (typeof node[j] != 'object') { clone[i].push(node[j]); } else { clone[i].push(this.deepClone(node[j])); } } } else { clone[i] = this.deepClone(node); } } else { clone[i] = node; } }
        return clone;
    },
    isBoolean: function(bool) { return (bool === true || bool === false); },
    isRegExp: function(obj) { return !!(obj && obj.test && obj.exec && (obj.ignoreCase || obj.ignoreCase === false)); }
});
Object.extend(String.prototype, {
    cleanJSON: function() { return this.replace(/(\"?)(\:|\,)\s+(\"?)/g, '$1$2$3'); },
    ...
    sanitize: function() { var str = this; return (str + '').replace(/[\\"']/g, '\\$&').replace(/\u0000/g, '\\0'); },
    nl2br: function(is_xhtml) { var str = this; var breakTag = (is_xhtml || typeof is_xhtml === 'undefined') ? '<br />' : '<br>'; return (str + '').replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1' + breakTag + ''); },
    ...
    ucFirst: function() { return this.charAt(0).toUpperCase() + this.substr(1, this.length + 1); }
});
...
Object.extend(document, {
    createCSS: function(selector, declaration) {
        ...
        if (isIE && document.styleSheets && document.styleSheets.length > 0) { var last_style_node = document.styleSheets[document.styleSheets.length - 1]; if (typeof(last_style_node.addRule) == "object") { last_style_node.addRule(selector, declaration); } }
    },
    ...
    preloadImages: function(images) {
        var args = arguments;
        if (Object.isArray(images)) { args = images; }
        var i = 0;
        for (i = 0, images = [];
            (src = args[i]); i++) {
    ...
    parameters: __result,
    get: __result,
    ready: function(func) { document.observe("dom:loaded", func); },
    getUnderneathElement: function(e) { var pointX = (Prototype.Browser.WebKit) ? Event.pointerX(e) : e.clientX; var pointY = (Prototype.Browser.WebKit) ? Event.pointerY(e) : e.clientY; return document.elementFromPoint(pointX, pointY); },
    ...
    eraseCookie: function(name) { document.createCookie(name, "", -1); },
    storeJsonCookie: function(name, value, days) {
        var val = Object.toJSON(value).cleanJSON();
        document.createCookie(name, val, days);
    },
    readJsonCookie: function(name) { if (document.readCookie(name)) { return document.readCookie(name).toString().evalJSON(); } else { return {}; } },
    getClientDimensions: function() { var head = document.body.parentNode; return { height: head.scrollHeight, width: head.scrollWidth }; },
    keyboardMap: function(map) {
        ...
        var shortcut = {
            ...
            'add': function(shortcut_combination, callback, opt) {
                ...
                var func = function(e) {
                    e = e || window.event;
                    if (opt.disable_in_input) { var element; if (e.target) { element = e.target; } else if (e.srcElement) { element = e.srcElement; } if (element.nodeType == 3) { element = element.parentNode; } if (element.tagName == 'INPUT' || element.tagName == 'TEXTAREA' || element.readAttribute('contenteditable') || document._onedit) { return; } }
                    ...
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        } else if (k.length > 1) { if (special_keys[k] == code) { kp++; } } else if (opt.keycode) { if (opt.keycode == code) { kp++; } } else { if (character == k) { kp++; } else { if (shift_nums[character] && e.shiftKey) { character = shift_nums[character]; if (character == k) { kp++; } } } }
                    }
        ...
        $H(map).each(function(pair) {
            var key = pair.key;
            var opts = pair.value;
            shortcut.add(key, opts.handler, { disable_in_input: opts.disableOnInputs });
        });
    },
...
Object.extend(Event, {
    ...
    isRightClick: function(event) {
        ...
        } else { _isButton = function(event, code) { return event.which ? (event.which === code + 1) : (event.button === code); }; }
...
Protoplus.utils = {
    ...
    openInNewTab: function(element, link) {
        element.observe('mouseover', function(e) {
            if (!element.tabLink) {
                ...
                element.tabLink = a;
                element.observe('mousemove', function(e) { element.tabLink.setStyle('top:' + (Event.pointerY(e) - 2.5) + 'px;left:' + (Event.pointerX(e) - 2.5) + 'px'); });
            }
        });
        return element;
    },
    hasFixedContainer: function(element) {
        ...
        element.ancestors().each(function(el) {
            if (result) { return; }
            if (el.style.position == "fixed") { result = true; }
        });
        return result;
    },
    getCurrentStyle: function(element, name) {
        if (element.style[name]) { return element.style[name]; } else if (element.currentStyle) { return element.currentStyle[name]; } else if (document.defaultView && document.defaultView.getComputedStyle) {
    ...
    isOverflow: function(element) {
        if (element.resized) { element.hideHandlers(); }
        ...
        if (element.resized) { element.showHandlers(); }
        return isOverflowing ? { top: topOverflowing ? element.scrollHeight : false, left: leftOverflowing ? element.scrollWidth : false, both: leftOverflowing && topOverflowing } : false;
    },
    setUnselectable: function(target) {
        if (typeof target.style.MozUserSelect != "undefined" && target.className == "form-section-closed") { target.style.MozUserSelect = "normal"; } else if (typeof target.onselectstart != "undefined") { target.onselectstart = function() { return false; }; } else if (typeof target.style.MozUserSelect != "undefined") { target.style.MozUserSelect = "none"; } else { target.onmousedown = function() { return false; }; }
        target.__oldCursor = target.style.cursor;
        target.style.cursor = 'default';
        return target;
    },
    setSelectable: function(target) {
        if (typeof target.onselectstart != "undefined") { target.onselectstart = document.createElement("div").onselectstart; } else if (typeof target.style.MozUserSelect != "undefined") { target.style.MozUserSelect = document.createElement("div").style.MozUserSelect; } else { target.onmousedown = ""; }
    ...
    hover: function(elem, over, out) {
        $(elem).observe("mouseover", function(evt) {
            if (typeof over == "function") {
                if (elem.innerHTML) { if (elem.descendants().include(evt.relatedTarget)) { return true; } }
                over(elem, evt);
            } else if (typeof over == "string") { $(elem).addClassName(over); }
        });
        $(elem).observe("mouseout", function(evt) {
            if (typeof out == "function") {
                if (elem.innerHTML) { if (elem.descendants().include(evt.relatedTarget)) { return true; } }
                out(elem, evt);
            } else if (typeof over == "string") { $(elem).removeClassName(over); }
        });
        return elem;
    },
    mouseEnter: function(elem, over, out) {
        $(elem).observe("mouseenter", function(evt) { if (typeof over == "function") { over(elem, evt); } else if (typeof over == "string") { $(elem).addClassName(over); } });
        $(elem).observe("mouseleave", function(evt) { if (typeof out == "function") { out(elem, evt); } else if (typeof over == "string") { $(elem).removeClassName(over); } });
        return elem;
    },
    setScroll: function(element, amounts) {
        if (amounts.x !== undefined) { element.scrollLeft = amounts.x; }
        if (amounts.y !== undefined) { element.scrollTop = amounts.y; }
    },
    scrollInto: function(element, options) {
        options = Object.extend({ offset: [100, 100], direction: 'bottom' }, options || {});
        element = $(element);
        var pos = Element.cumulativeOffset(element);
        ...
        switch (options.direction) {
            ...
            case "top":
                ...
                if (window.scrollY !== 0 && pos[1] <= window.scrollY + options.offset[1]) { window.scrollTo(window.scrollX, pos[1] - options.offset[1]); } else if (window.scrollY !== 0 && (pos[1] + options.offset[1] <= Math.abs(vp.height - window.scrollY))) { window.scrollTo(window.scrollX, pos[1] - options.offset[1]); }
                break;
        }
        return element;
    },
    getScroll: function(element) { return { x: parseFloat(element.scrollLeft), y: parseFloat(element.scrollTop) }; },
    setText: function(element, value) { element.innerHTML = value; return element; },
    putValue: function(element, value) {
        if (element.clearHint) { element.clearHint(); }
        element.value = value;
        return element;
    },
    ...
    run: function(element, event) {
        if (event.include(':')) { element.fire(event); } else {
            var evt;
            ...
            if (document.createEventObject && !Prototype.Browser.IE9 && !Prototype.Browser.IE10) {
                ...
                } else { element.dispatchEvent(evt); }
            }
        }
        return element;
    },
    setCSSBorderRadius: function(element, value) { return element.setStyle({ MozBorderRadius: value, borderRadius: value, '-webkit-border-radius': value }); },
    getSelected: function(element) {
        if (!element.options) {
            if (element.innerHTML) { return element.innerHTML; } else { return element.value; }
        }
    ...
    stopAnimation: function(element) { element.__stopAnimation = true; return element; },
    shift: function(element, options) {
        options = Object.extend({ duration: 1, onEnd: Prototype.K, onStart: Prototype.K, onStep: Prototype.K, onCreate: Prototype.K, delay: 0, link: 'cancel', transparentColor: '#ffffff', remove: false, easingCustom: false, propertyEasings: {}, easing: Protoplus.Transitions.sineOut }, options || {});
        if (!element.queue) { element.queue = []; }
        ...
        if (typeof options.easing == 'string') { if (options.easing in Protoplus.Transitions) { options.easing = Protoplus.Transitions[options.easing]; } else { options.easing = Protoplus.Transitions.sineOut; } } else if (typeof options.easing == 'object') {
            options.propertyEasings = options.easing;
            options.easing = Protoplus.Transitions.sineOut;
        } else if (typeof options.easing != 'function') { options.easing = Protoplus.Transitions.sineOut; }
        ...
        for (var x in options) { if (!["duration", "onStart", "onStep", "transparentColor", "onEnd", "onCreate", "remove", "easing", "link", "delay", "easingCustom", "propertyEasings"].include(x) && options[x] !== false) { properties[x] = options[x]; } }
        ...
        for (var i in properties) {
            ...
            if (["scrollX", "scrollLeft", "scrollY", "scrollTop"].include(okey)) {
                ...
                if (element.tagName == "BODY") { from = (okey == "scrollX" || okey == "scrollLeft") ? window.scrollX : window.scrollY; } else { from = (okey == "scrollX" || okey == "scrollLeft") ? element.scrollLeft : element.scrollTop; }
                unit = '';
            } else if (okey == "rotate") {
                to = parseFloat(oval);
            ...
            } else { properties[okey] = { key: key, to: to, from: from, unit: unit, easing: easing }; }
        }
        var fn = function(ease, option, arr) {
            var val = 0;
            if (arr !== false) { return Math.round(option.from[arr] + ease * (option.to[arr] - option.from[arr])); }
        ...
        var step = function() {
            ...
            if (time >= end) {
                ...
                for (okey in properties) { oval = properties[okey]; if (["scrollX", "scrollLeft", "scrollY", "scrollTop"].include(okey)) { if (element.tagName.toUpperCase() == "BODY") { if (oval.key == "scrollLeft") { window.scrollTo(oval[valTo], window.scrollY); } else { window.scrollTo(window.scrollX, oval[valTo]); } } else { element[oval.key] = oval[valTo] + oval.unit; } } else if (["background", "color", "borderColor", "backgroundColor"].include(okey)) { element.style[oval.key] = 'rgb(' + oval[valTo].join(', ') + ")"; } else if (okey == "opacity") { Element.setOpacity(element, oval[valTo]); } else if (okey == "rotate") { element.style[okey] = "rotate(" + oval[valTo] + oval.unit + ")"; } else { element.style[okey] = oval[valTo] + oval.unit; } }
    ...
    fade: function(element, options) {
        options = Object.extend({ duration: 0.5, onEnd: function(e) { e.setStyle({ display: "none" }); }, onStart: Prototype.K, opacity: 0 }, options || {});
        element.shift(options);
    },
    appear: function(element, options) {
        options = Object.extend({ duration: 0.5, onEnd: Prototype.K, onStart: Prototype.K, opacity: 1 }, options || {});
    ...
    getReference: function(element, name) {
        if (!element.REFID) { return false; }
        return Protoplus.references[element.REFID][name];
    },
    remove: function(element) {
        if (element.REFID) { delete Protoplus.references[element.REFID]; }
...
(function(emile, container) {
    ...
    container[emile] = function(el, style, opts) {
        ...
        var target = normalize(style),
            ...
            finish = start + dur,
            interval, easing = opts.easing || function(pos) { return (-Math.cos(pos * Math.PI) / 2) + 0.5; };
        for (prop in target) { current[prop] = parse(comp[prop]); }
        interval = setInterval(function() {
            var time = +new Date(),
                pos = time > finish ? 1 : (time - start) / dur;
            for (var prop in target) { el.style[prop] = target[prop].f(current[prop].v, target[prop].v, easing(pos)) + target[prop].u; }
            if (time > finish) { clearInterval(interval); if (opts.after) { opts.after(); } }
        }, 10);
    };
})('emile', Protoplus.utils);
Element.addMethods(Protoplus.utils);
Event.observe(window, 'unload', function() { Protoplus = null; });
Ajax = Object.extend(Ajax, {
    Jsonp: function(url, options) {
        this.options = Object.extend({ method: 'post', timeout: 60, parameters: '', force: false, onComplete: Prototype.K, onSuccess: Prototype.K, onFail: Prototype.K }, options || {});
        var parameterString = url.match(/\?/) ? '&' : '?';
        this.response = false;
        var callback_id = new Date().getTime();
        Ajax["callback_" + callback_id] = function(response) { this.response = response; }.bind(this);
        this.callback = Ajax.callback;
        if (typeof this.options.parameters == "string") { parameterString += this.options.parameters; } else { $H(this.options.parameters).each(function(p) { parameterString += p.key + '=' + encodeURIComponent(p.value) + '&'; }); }
        var matches = /^(\w+:)?\/\/([^\/?#]+)/.exec(url);
        var sameDomain = (matches && (matches[1] && matches[1] != location.protocol || matches[2] != location.host));
        if (!sameDomain && this.options.force === false) { return new Ajax.Request(url, this.options); }
        ...
        this.onLoad = function(e) {
            ...
            } else { this.onError({ error: 'Callback error' }); }
        }.bind(this);
        this.readyState = function(e) { var rs = this.script.readyState; if (rs == 'loaded' || rs == 'complete') { this.onLoad(); } }.bind(this);
...
if (!location.pathname.match(/^\/answers\/.+/)) {
    window.alert = function() {
        ...
        var i = 1;
        var first = args[0];
        if (typeof first == "object") { $H(first).debug(); return first; } else if (typeof first == "string") {
            var msg = first.replace(/(\%s)/gim, function(e) { return args[i++] || ""; });
            _alert(msg);
            return true;
        }
        _alert(first);
...
if ("__protoinit" in window) { document.ready(function(e) { $A(__protoinit).each(function(f) { f(e); }); }); }
(function() {
    if (Prototype.Browser.WebKit) {
        var FIX_WEBKIT_FOCUS = function(e) { if (e.target && e.target.focus) { e.target.focus(); } };
        document.addEventListener('DOMNodeInserted', function(e) { if (e.target.tagName === 'BUTTON' || (e.target.tagName === 'INPUT' && e.target.type !== 'text')) { e.target.observe('click', FIX_WEBKIT_FOCUS); } }, false);
        document.observe('dom:loaded', function() { $$('button, input:not([type="text"])').invoke('observe', 'click', FIX_WEBKIT_FOCUS); });
    }
})();;
...
Object.extend(document, {
    ...
    startTooltips: function() { document.stopTooltip = false; },
    windowDefaults: { height: false, width: 400, title: '&nbsp;', titleBackground: '#F5F5F5', buttonsBackground: '#F5F5F5', background: '#FFFFFF', top: '25%', left: '25%', position: 'absolute', winZindex: 10001, borderWidth: 10, borderColor: '#000', titleTextColor: '#777', resizable: false, borderOpacity: 0.3, borderRadius: "5px", titleClass: false, contentClass: false, buttonsClass: false, closeButton: 'X', fullScreenButton: '<img src="images/wizard-fullscreen.png" style="width:18px; height:18px;" class="fullscreen-wiz" title="Full Screen">', allowFullScreen: false, defaultFullScreen: false, openEffect: true, closeEffect: true, dim: true, modal: true, dimColor: '#fff', dimOpacity: 0.8, dimZindex: 10000, dynamic: true, contentPadding: '8', closeTo: false, buttons: false, buttonsAlign: 'right', hideTitle: false },
    window: function(options) {
        var topOverridden = ("top" in options);
        options = Object.extend(Object.deepClone(document.windowDefaults), options || {});
        options = Object.extend({ onClose: Prototype.K, onInsert: Prototype.K, onDisplay: Prototype.K, onFullScreen: Prototype.K, onUnFullScreen: Prototype.K }, options, {});
        ...
        if (options.dim) {
            ...
            dimmer.observe('click', function() { closebox(); })
            dimmer.onmousedown = function() { return false; };
        ...
        win.setTitle = function(title) { title_text.update(title); return win; };
        ...
        win.unblockWindow = function() { block.remove(); return win; };
        ...
        if (options.buttons && options.buttons.length > 0) {
            ...
            if (!options.buttonsClass) { buttons.setStyle(buttonsStyle); } else { buttons.addClassName(options.buttonsClass); }
            ...
            $A(options.buttons).each(function(button, i) {
                ...
                var but = new Element('button', { className: 'big-button buttons buttons-' + color, type: 'button', name: button.name || "button-" + i, id: button.id || "button-" + i }).observe('click', function() { button.handler(win, but); });
                ...
                if (button.icon) {
                    ...
                    var butIcon = new Element('img', { src: button.icon, align: button.iconAlign == 'right' ? 'absmiddle' : 'left' }).addClassName("icon-" + button.iconAlign);
                    if (button.iconAlign == 'left') { but.insert(butIcon); }
                    but.insert(butTitle);
                    if (button.iconAlign == 'right') { but.insert(butIcon); }
                } else { but.insert(butTitle); }
                if (button.align == 'left') { but.setStyle('float:left'); }
                but.changeTitle = function(title) { butTitle.update(title); return but; };
        ...
        } else { contentWrapper.setStyle({ MozBorderRadius: buttonsStyle.MozBorderRadius, WebkitBorderRadius: buttonsStyle.WebkitBorderRadius, borderRadius: buttonsStyle.borderRadius }); }
        win.setStyle(windowStyle);
        background.setStyle(backgroundStyle).setCSSBorderRadius(options.borderRadius);
        if (!options.titleClass) { title.setStyle(titleStyle); } else { title.addClassName(options.titleClass); }
        ...
        var closebox = function(key) {
            ...
            if (options.onClose(win, key) !== false) {
                ...
                if (options.closeEffect === true) { win.shift({ opacity: 0, duration: 0.3, onEnd: close }); } else { close(); }
        ...
        var fullScreenGeneric = function() {
            ...
            win.down('.window-content').setStyle({ height: parseInt(document.viewport.getHeight()) - (parseInt($$('.window-buttons-wrapper')[0].getHeight()) + 40) + 'px' });
            document.documentElement.style.overflow = 'hidden';
            document.body.scroll = "no";
            window.onresize = function(e) { options.onFullScreen(); }
        ...
        var unFullScreenGeneric = function() {
            ...
            win.down('.window-content').setStyle({ height: 'auto', position: 'relative' });
            win.reCenter();
            window.onresize = null;
            document.documentElement.style.overflow = 'visible';
            document.body.scroll = "yes";
            options.onUnFullScreen();
        };
        if (options.allowFullScreen) {
            title_fullscreen.insert(options.fullScreenButton);
            title_fullscreen.onclick = function() {
                if (fullScreened) { unFullScreenGeneric(); } else { fullScreenGeneric(); }
        ...
        win.reCenter = function() {
            ...
            if (topOverridden) { top = options.top.toString(); if (top.indexOf('%') === -1 && top.indexOf('px') === -1) { top += "px"; } } else { top += "px"; }
            win.setStyle({ top: top, left: left + "px" });
            if (dimmer) { dimmer.setStyle({ height: vp.height + 'px', width: vp.width + 'px' }); }
        };
        win.reCenter();
        ...
        if (options.resizable) {
            wrapper.resizable({
                ...
                onResize: function(h, w, type) {
                    if (type != 'vertical') { win.setStyle({ width: (w + (options.borderWidth * 2) - 10) + 'px' }); }
        ...
        if (options.defaultFullScreen) { fullScreenGeneric(); }
        return win;
    }
});
document.observe('keyup', function(e) {
    e = document.getEvent(e);
    if (Event.isInput(e)) { return; }
    if (document.openWindows.length > 0) { if (e.keyCode == 27) { document.openWindows.pop().close('ESC'); } }
});
...
Protoplus.ui = {
        ...
        editable: function(elem, options) {
            ...
            try { elem.innerHTML = elem.innerHTML || elem.defaultText; } catch (e) {}
            var clickareas = [elem];
            ...
            $A(clickareas).invoke('observe', options.doubleClick ? "dblclick" : "click", function(e) {
                ...
                var blur = function(e) {
                    if (elem.keyEventFired) { elem.keyEventFired = false; return; }
                ...
                if (type.toLowerCase() == "textarea") { currentValue = currentValue.replace(/<br>/gi, "&lt;br&gt;"); }
                ...
                if (type.toLowerCase() == "textarea") {
                    ...
                    if (typeof op[0] == "string") { op.each(function(text) { input.insert(new Element("option").insert(text)); }); } else { op.each(function(pair, i) { input.insert(new Element("option", { value: pair.value ? pair.value : i }).insert(pair.text)); }); }
                    input.selectOption(currentValue);
                    input.observe("blur", blur);
                } else if (["radio", "checkbox"].include(type.toLowerCase())) { input = new Element("div"); if (typeof op[0] == "string") { op.each(function(text, i) { input.insert(new Element("input", { type: type, name: "pp", id: "pl_" + i })).insert(new Element("label", { htmlFor: "pl_" + i, id: "lb_" + i }).insert(text)).insert("<br>"); }); } else { op.each(function(pair, i) { input.insert(new Element("input", { type: type, name: "pp", value: pair.value ? pair.value : i, id: "pl_" + i })).insert(new Element("label", { htmlFor: "pl_" + i, id: "lb_" + i }).insert(pair.text)).insert("<br>"); }); } } else {
            ...
            var finish = function(e, oldValue) {
                ...
                outer.onedit = false;
                if ("select" == elem.nodeName.toLowerCase()) { val = elem.options[elem.selectedIndex].text; } else if (["checkbox", "radio"].include(elem.type && elem.type.toLowerCase())) {
                    outer = $(elem.parentNode.parentNode);
                    val = "";
                    $(elem.parentNode).descendants().findAll(function(el) { return el.checked === true; }).each(function(ch) { if ($(ch.id.replace("pl_", "lb_"))) { val += $(ch.id.replace("pl_", "lb_")).innerHTML + "<br>"; } });
                } else { val = elem.value; }
                ...
                try {
                    ...
                    setTimeout(function() { if ($(outer.parentNode.parentNode).__drag_just_after_add !== true) { outer.onEnd(outer, outer.innerHTML, oldValue, selected || val); } else { $(outer.parentNode.parentNode).__drag_just_after_add_on_end_relay = function() { outer.onEnd(outer, outer.innerHTML, oldValue, elem.getSelected() || val); }; } }, 1);
                } catch (err) {}
            };
            return elem;
        },
        setShadowColor: function(elem, color) {
            elem = $(elem);
            $A(elem.descendants()).each(function(node) { if (node.nodeType == Node.ELEMENT_NODE) { node.setStyle({ color: color }); } });
            return elem;
        },
        cleanShadow: function(elem) {
            elem = $(elem);
            elem.descendants().each(function(e) { if (e.className == "pp_shadow") { e.remove(); } });
            return elem;
        },
        ...
        hasContextMenu: function(element) { return !!element._contextMenuEnabled; },
        setContextMenu: function(element, options) {
            element = $(element);
            options = Object.extend({ others: [] }, options || {});
            element._contextMenuEnabled = true;
            element.items = {};
            $A(options.menuItems).each(function(item, i) { if (item == '-') { element.items["seperator_" + i] = item; } else { if (!item.name) { element.items["item_" + i] = item; } else { element.items[item.name] = item; } } });
            element.changeButtonText = function(button, text) { element.items[button].title = text; return $(element.items[button].elem).select('.context-menu-item-text')[0].update(text); };
            ...
            var getContPosition = function(container) {
                ...
                if (!isNotFit && p && p.__pos) { pos = p.__pos; } else if (isNotFit) { pos = "left"; } else { pos = "right"; }
                container.__pos = pos;
                return pos;
            };
            var createInnerList = function(cont, itemConf) {
                ...
                if (Object.isFunction(itemConf.items)) { it = itemConf.items(); }
                $A(it).each(function(item) { var liItem = createListItem(context, item); });
            ...
            var openMenu = function(e, local, opt) {
                ...
                if (local || (Prototype.Browser.Opera && e.ctrlKey) || Event.isRightClick(e) || Prototype.Browser.IE) {
                    ...
                    if (element !== false) {
                        ...
                        container.setStyle({ position: 'absolute', top: (opt.top ? opt.top : top) + 'px', left: (opt.left ? opt.left : left) + 'px' });
            ...
            if (!document.contextMenuHandlerSet) {
                ...
                $(document).observe('click', function(e) { $$('.context-menu-all').invoke('remove'); });
            }
            return element;
        },
        textshadow: function(element, options) {
            element = $(element);
            options = Object.extend({ light: "upleft", color: "#666", offset: 1, opacity: 1, padding: 0, glowOpacity: 0.1, align: undefined, imageLike: false }, options || {});
            ...
            var style = { color: color, height: element.getStyle("height"), width: element.getStyle("width"), "text-align": textalign, padding: padding, position: "absolute", "z-index": 100, opacity: opacity };
            ...
            for (var i = 0; i < dist; i++) {
                ...
                switch (light) {
                    ...
                    case "glow":
                        shadowdiv.setStyle({ top: "0px", left: "0px" });
                        container.appendChild(new Element("div").setStyle(Object.extend(style, { top: d + "px", opacity: options.glowOpacity })).update(text).setShadowColor(color).setUnselectable());
                        container.appendChild(new Element("div").setStyle(Object.extend(style, { top: "-" + d + "px", opacity: options.glowOpacity })).update(text).setShadowColor(color).setUnselectable());
                        container.appendChild(new Element("div").setStyle(Object.extend(style, { top: d + "px", left: "-" + d + "px", opacity: options.glowOpacity })).update(text).setShadowColor(color).setUnselectable());
                        container.appendChild(new Element("div").setStyle(Object.extend(style, { top: d + "px", left: d + "px", opacity: options.glowOpacity })).update(text).setShadowColor(color).setUnselectable());
                        container.appendChild(new Element("div").setStyle(Object.extend(style, { top: "-" + d + "px", left: "-" + d + "px", opacity: options.glowOpacity })).update(text).setShadowColor(color).setUnselectable());
                        container.appendChild(new Element("div").setStyle(Object.extend(style, { top: "-" + d + "px", left: d + "px", opacity: options.glowOpacity })).update(text).setShadowColor(color).setUnselectable());
                        break;
            ...
            if (options.imageLike) { elem.setUnselectable().setStyle({ cursor: "default" }); }
            return element;
        },
        tooltip: function(element, text, options) {
            element = $(element);
            if ('Prototip' in window) { options = Object.extend({ delay: 0.01 }, options || {}); var T = new Tip(element, text, options); return element; }
            if (typeof text != "string") { return element; }
            options = Object.extend({ className: false, fixed: false, opacity: 1, title: false, width: 200, height: 100, offset: false, zIndex: 100000, delay: false, duration: false, fadeIn: false, fadeOut: false, shadow: false }, options || {});
            ...
            element.hover(function(el, evt) {
                ...
                if (document.stopTooltip) { $$(".pp_tooltip_").each(function(t) { t.remove(); }); return true; }
                outer = new Element("div", { className: 'pp_tooltip_' }).setStyle({ opacity: options.opacity, position: "absolute", zIndex: options.zIndex });
                if (options.className) { tooldiv = new Element("div", { className: options.className }).setStyle({ position: "relative", top: "0px", left: "0px", zIndex: 10 }).update(text); } else {
        ...
        softScroll: function(element, options) {
            options = Object.extend({ scrollSpeed: 50 }, options || {});
            ...
            element.updateScrollSize = function() {
                var ch;
                try { ch = container.measure('margin-box-height'); } catch (e) {
                ...
                if (sh <= 0) { return; }
                var per = ch * 100 / sh;
                if (per > 100) { scroll.hide(); } else { scroll.show(); }
            ...
            scroll.setDraggable({
                ...
                onDrag: function(el) {
                    ...
                    if (top < 0) { el.style.top = "0px"; return false; }
                    var h = container.getHeight();
                    var sh = scroll.getHeight();
                    if ((top + sh) > h) { el.style.top = (h - (sh)) + "px"; return false; }
                    scrollArea(top);
                }
            });
        ...
        setDraggable: function(element, options) {
            ...
            var drag = function(e) {
                Event.stop(e);
                if (mouseMove == "touchmove") { e = e.touches[0]; }
                ...
                if (options.constraint == "vertical") { drag_element.setStyle({ top: top + "px" }); } else if (options.constraint == "horizontal") { drag_element.setStyle({ left: left + "px" }); } else { drag_element.setStyle({ top: top + "px", left: left + "px" }); }
                if (stopDragTimer) { clearTimeout(stopDragTimer); }
                options.onDrag(drag_element, handler, e);
                stopDragTimer = setTimeout(function() { options.onDragEnd(drag_element, handler, e); }, 50);
            };
            var mouseup = function(ev) {
                Event.stop(ev);
                if (mouseUp == "touchend") { ev = e.touches[0]; }
                ...
                if (options.onEnd(drag_element, handler, ev) !== false) {
                    if (element.oldZIndex) { drag_element.setStyle({ zIndex: element.oldZIndex }); } else { drag_element.setStyle({ zIndex: '' }); }
                    if (options.revert) {
                        if (options.revert === true) { options.revert = { easing: "sineIn", duration: 0.5 }; }
                        options.revert = Object.extend({ left: drag_element.startX, top: drag_element.startY, opacity: 1, duration: 0.5, easing: 'sineIn' }, options.revert || {});
                        drag_element.shift(options.revert);
                        drag_element.startX = false;
                        drag_element.startY = false;
                    } else { if (options.dragEffect) { drag_element.shift({ opacity: 1, duration: 0.2 }); } }
                }
            ...
            if (options.handler) { if (typeof options.handler == "string") { handler = (options.handler.startsWith(".")) ? element.descendants().find(function(h) { return h.className == options.handler.replace(/^\./, ""); }) : $(options.handler); } else { handler = $(options.handler); } } else { handler = element; }
            handler.setStyle({ cursor: options.cursor });
            handler.observe(mouseDown, function(e) {
                Event.stop(e);
                var evt = e;
                if (mouseDown == "touchstart") { e = e.touches[0]; }
                ...
                if (options.constrainParent) {
                    if ($(element.parentNode).getStyle('position') == "relative" || $(element.parentNode).getStyle('position') == "absolute") { voff = { top: 0, left: 0 }; } else { voff = (Prototype.Browser.IE) ? { top: 0, left: 0 } : $(element.parentNode).cumulativeOffset(); }
                ...
                if (options.dynamic !== true) {
                    try {
                        ...
                        temp_div = new Element('div').setStyle({ height: element.getHeight() + "px", width: element.getWidth() + "px", border: '1px dashed black', top: element.getStyle('top') || 0, left: element.getStyle('left') || 0, zIndex: element.getStyle('zIndex') || 0, position: element.getStyle('position'), background: '#f5f5f5', opacity: 0.3 });
                    } catch (err) {}
                ...
                if (options.dragEffect) { drag_element.shift({ opacity: 0.7, duration: 0.2 }); }
        ...
        rating: function(element, options) {
            ...
            element.setStyle({ display: 'inline-block', width: ((parseInt(options.stars, 10) + (options.resetButton ? 1 : 0)) * 20) + "px", cursor: options.disabled ? "default" : "pointer" });
            ...
            var setStar = function(i) {
                ...
                element.run('keyup');
                hidden.run('change');
                if (options.resetButton) { cross[(i === 0) ? "hide" : "show"](); }
            };
            ...
            $A($R(1, options.stars)).each(function(i) {
                ...
                }).observe("click", function() { if (!element.disabled) { setStar(i); } });
                if (options.titles && options.titles[i - 1]) { star.title = options.titles[i - 1]; }
                stardivs.push(star);
            });
            if (!options.disabled) { element.observe("mouseout", function() { element.descendants().each(function(e) { e.setStyle({ backgroundPosition: e.hasClassName("rated") ? image.clicked : image.blank }); }); }); }
            if (options.resetButton) {
                ...
                if (options.resetButtonImage) { cross.insert(new Element('img', { src: options.resetButtonImage, align: 'absmiddle' })); } else { cross.insert(' x '); }
                cross.title = options.resetButtonTitle;
                cross.hide();
                cross.observe('click', function() { setStar(undefined); });
                stardivs.push(cross);
            }
            ...
            if (options.selectedRating > 0) {
                element.descendants().each(function(e, c) {
                    c++;
                    if (c <= options.selectedRating) { e.setStyle({ backgroundPosition: image.clicked }).addClassName("rated"); }
                    if (options.selectedRating > c - 1 && options.selectedRating < c) { e.setStyle({ backgroundPosition: image.half }).addClassName("rated"); }
                });
        ...
        makeSearchBox: function(element, options) {
            element = $(element);
            if (element.up('.searchbox')) { return element; }
            options = Object.extend({ defaultText: "Search", onWrite: Prototype.K, onClear: Prototype.K, imagePath: "images/apple_search.png" }, options || {});
            element.observe("keyup", function(e) {
                if (cross) { cross.setStyle({ backgroundPosition: element.value !== "" ? "0 -57px" : "0 -38px" }); }
            ...
            if (element.type !== 'text') {
                ...
                element.observe('search', function() { element.run('keyup'); });
                return element;
            }
            element.setStyle({ border: "none", background: "none", height: "14px", outline: 'none', width: (parseInt(element.getStyle("width"), 10) - 32) + "px" });
            var tbody;
            var table = new Element("table", { cellpadding: 0, cellspacing: 0, className: "searchbox" }).setStyle({ height: "19px", fontFamily: "Verdana, Geneva, Arial, Helvetica, sans-serif", fontSize: "12px" }).insert(tbody = new Element("tbody"));
            var tr = new Element("tr");
            var cont = new Element("td").setStyle({ backgroundImage: "url(" + options.imagePath + ")", backgroundPosition: "0 -19px" });
            var cross = new Element("td").insert("&nbsp;").setStyle({ cursor: 'default' });
            tbody.insert(tr.insert(new Element("td").setStyle({ backgroundImage: "url(" + options.imagePath + ")", backgroundPosition: "0 0", width: "10px" }).insert("&nbsp;")).insert(cont).insert(cross));
            cross.setStyle({ backgroundImage: "url(" + options.imagePath + ")", backgroundPosition: element.value !== "" ? "0 -57px" : "0 -38px", width: "17px" });
        ...
        slider: function(element, options) {
            element = $(element);
            options = Object.extend({ width: 100, onUpdate: Prototype.K, maxValue: 100, value: 0, buttonBack: 'url("../images/ball.png") no-repeat scroll 0px 0px transparent' }, options || {});
            if ("JotForm" in window && "url" in JotForm) { options.buttonBack = 'url("' + JotForm.url + 'images/ball.png") no-repeat scroll 0px 0px transparent'; }
            ...
            var sliderKeys = function(e) { e = document.getEvent(e); if (e.keyCode == 37) { moveLEFT(5); } else if (e.keyCode == 39) { moveRIGTH(5); } };
            var sliderWheel = function(e) {
                if (!sliderOut.__hasFocus) { return true; }
                e.stop();
                sliderOut.focus();
                var w = Event.wheel(e);
                if (w > 0) { moveRIGTH(5); } else if (w < 0) { moveLEFT(5); }
            };
            ...
            sliderOut.setStyle({ width: options.width + 'px', position: 'relative', overflow: 'hidden', outline: 'none' });
            sliderBar.setStyle({ border: '1px solid #999', background: '#eee', margin: '8px', overflow: 'hidden', height: '3px' }).setCSSBorderRadius('4px');
            sliderButton.setStyle({ position: 'absolute', height: '13px', width: '13px', background: options.buttonBack, overflow: 'hidden', border: '1px solid transparent', top: '3px', left: options.value + 'px' }).setCSSBorderRadius('8px');
            ...
            sliderButton.setDraggable({ constraint: 'horizontal', dragEffect: false, cursor: 'default', constrainLeft: 3, constrainRight: barWidth, onDrag: function(i) { updateValue(i.getStyle('left')); } });
        ...
        spinner: function(element, options) {
            element = $(element);
            options = Object.extend({ width: 60, cssFloat: false, allowNegative: false, addAmount: 1, maxValue: false, minValue: false, readonly: false, value: false, allowEmpty: false, size: 5, imgPath: 'images/', onChange: Prototype.K }, options || {});
            element.size = options.size;
            if (options.value === false) { if (!(options.allowEmpty && element.value === "")) { element.value = parseFloat(element.value) || '0'; } } else { element.value = options.value; }
            if (options.minValue) {
            ...
            if (options.cssFloat) { spinnerContainer.setStyle({ cssFloat: options.cssFloat, marginRight: '5px' }); }
            ...
            spinnerTable.setStyle({ border: '1px solid #ccc', borderCollapse: 'collapse', background: '#fff' });
            ...
            var numberDOWN = function(e) {
                ...
                if (options.minValue) { if (Number(newValue) < Number(options.minValue)) { return; } } else if (!options.allowNegative && newValue < 0) { return; }
            ...
            var spinnerKeys = function(e, mode) {
                ...
                e = document.getEvent(e);
                if (e.keyCode == 38) { numberUP(e); } else if (e.keyCode == 40) { numberDOWN(e); }
            };
            ...
            if (options.readonly) { element.writeAttribute('readonly', "readonly"); }
            element.observe('change', function() { options.onChange(element.value); });
            return element;
        },
        colorPicker: function(element, options) {
            options = Object.extend({ title: 'Pick a Color', background: '#eee', trigger: false, onPicked: Prototype.K, onComplete: Prototype.K, onStart: Prototype.K, onEnd: Prototype.K }, options || {});

            function sortColors(cols) {
                var obj = {};
                $H(cols).sortBy(function(p) { var rgb = Protoplus.Colors.hexToRgb(p.value); return rgb[0] + rgb[1] + rgb[2]; }).each(function(item) { obj[item[0]] = item[1]; });
                return obj;
            }
            $(options.trigger || element).observe('click', function() {
                if (options.onStart() === false) { element.colorPickerEnabled = false; return element; }
                ...
                if (options.className) { table.addClassName(options.className); } else { table.setStyle({ background: options.background, outline: '1px solid #aaa', border: '1px solid #fff' }); }
                tbody.insert(tr = new Element('tr').insert(new Element('th', { className: 'titleHandler', colspan: '2', height: '10' }).setText(options.title).setStyle({ paddingTop: '2px', paddingBottom: '0px', color: '#333', fontSize: '14px' }))).insert(colorTR = new Element('tr')).insert(selectTR = new Element('tr'));
                ...
                box.observe('keyup', function() { box.setStyle({ background: box.value, color: Protoplus.Colors.invert(box.value) }); });
                ...
                if (options.buttonClass) { $(flip, OK).invoke('addClassName', options.buttonClass); } else { $(flip, OK).invoke('setStyle', { padding: '1px', margin: '1px', background: '#f5f5f5', border: '1px solid #ccc' }); }
                element.closeColorPicker = function() { OK.run('click'); };
                ...
                $H(validCSSColors).each(function(color) {
                    ...
                    if (color.value === false) { colTR.insert(new Element('td', { width: tdSize, height: tdSize }).setStyle({ background: '#fff' }).setStyle({})); } else { colTR.insert(new Element('td', { width: tdSize, height: tdSize }).setStyle({ background: color.value }).observe('click', pick).tooltip(color.value, { delay: 0.6, width: 'auto' })); }
                });
        ...
        colorPicker2: function(element, options) {
            options = Object.extend({ onStart: Prototype.K, onEnd: Prototype.K, trigger: false, onPicked: Prototype.K, onComplete: Prototype.K, hideOnBlur: false, buttonClass: 'big-button buttons' }, options || {});
            var customColorHex;
            $(options.trigger || element).observe('click', function() {
                var docEvent = false;
                ...
                plugHEX.observe('mousedown', function(e) {
                    stop = 0;
                    setTimeout(function() { stop = 1; }, 100);
                });
                plugHEX.observe('keyup', function() { if (plugHEX.value.length >= 7) { setValue(plugHEX.value); } });
                plugHEX.observe('click', function() { Form.Element.select(this); });
                ...
                complete.observe('click', function() {
                    plugin.remove();
                    element.colorPickerEnabled = false;
                    if (docEvent) { docEvent.stop(); }
                    customColorHex = plugHEX.value;
                    options.onComplete(plugHEX.value);
                });
                element.closeColorPicker = function() { complete.run('click'); };
                ...
                function HSVupdate(v) {
                    ...
                    if (element.tagName == 'BUTTON') { element.__colorvalue = '#' + v; } else { element.value = '#' + v; }
                    options.onPicked('#' + v, element, plugin);
                    return (v);
                }

                function setValue(colorcode) {
                    if ("transparent".search(colorcode) == -1) {
                ...
                var color = {
                    ...
                    HEX: function(o) { o = Math.round(Math.min(Math.max(0, o), 255)); return ("0123456789ABCDEF".charAt((o - o % 16) / 16) + "0123456789ABCDEF".charAt(o % 16)); },
                    ...
                    RGB_HEX: function(o) { var fu = color.HEX; return (fu(o.R) + fu(o.G) + fu(o.B)); },
                    HSV_RGB: function(o) {
                    ...
                    HSV_HEX: function(o) { return (color.RGB_HEX(color.HSV_RGB(o))); }
                };
                $(document.body).insert(plugin);
                loadSV();
                if (customColorHex == 'transparent') { plugHEX.value = customColorHex; } else { setValue(element.__colorvalue || element.value || "#FFFFFF"); }
                options.onEnd(element, plugin);
                return element;
            });
        },
        miniLabel: function(element, label, options) {
            options = Object.extend({ position: 'bottom', color: '#666', size: 9, text: '', nobr: false }, options || {});
            element.wrap('span');
            ...
            var labelClick = function() { element.focus(); };
            var br = '<br>';
            if (options.nobr) { br = ''; }
        ...
        hint: function(element, value, options) {
            element = $(element);
            if ("placeholder" in element) { element.writeAttribute('placeholder', value); return element; }
            ...
            if (element.form) { $(element.form).observe('submit', submit); }
            ...
            element.removeHint = function() {
                ...
                if (element.form) { $(element.form).stopObserving('submit', submit); }
                return element;
            };
            return element;
        },
        resizable: function(element, options) {
            options = Object.extend({ sensitivity: 10, overflow: 0, onResize: Prototype.K, onResizeEnd: Prototype.K, imagePath: 'images/resize.png', element: false, maxHeight: false, minHeight: false, maxWidth: false, minWidth: false, maxArea: false, autoAdjustOverflow: true, constrainViewport: true, constrainParent: false, keepAspectRatio: false, displayHandlers: true }, options, {});
            ...
            if (!elementPos || elementPos == 'static') { handlerElem.setStyle({ position: 'relative' }); }
            var ratio;
            var firstDim = element.getDimensions();
            var paddings = { top: (parseInt(element.getStyle('padding-top'), 10) || 0) + (parseInt(element.getStyle('padding-bottom'), 10) || 0), left: (parseInt(element.getStyle('padding-left'), 10) || 0) + (parseInt(element.getStyle('padding-right'), 10) || 0) };
            ...
            var resize = function(e, type) {
                ...
                var sDim = $H(element.getDimensions()).map(function(d) {
                    if (d.key == "height") { return d.value - paddings.top; } else if (d.key == "width") { return d.value - paddings.left; }
                    return d.value;
                });
                var startDim = { height: sDim[1], width: sDim[0] };
                ...
                var setElementSize = function(dims) {
                    ...
                    if (height) {
                        ...
                        if (options.maxArea) { if (height * element.getWidth() >= options.maxArea) { return; } }
                    ...
                    if (width) {
                        ...
                        if (options.maxArea) { if (element.getHeight() * width >= options.maxArea) { return; } }
                ...
                var mousemove = function(e) {
                    ...
                    if (type != "vertical") {
                        ...
                        var wskip = false;
                        if (options.constrainViewport) { wskip = ((width + offs.left) >= (dim.width - 3)); }
                        ...
                        if (!wskip) { setElementSize({ width: width, type: type }); if (options.keepAspectRatio) { setElementSize({ height: width * ratio, type: type }); } }
                    }
                };
        ...
        positionFixed: function(element, options) {
            element = $(element);
            options = Object.extend({ offset: 10, onPinned: Prototype.K, onUnpinned: Prototype.K, onBeforeScroll: Prototype.K, onBeforeScrollFail: Prototype.K, onScroll: Prototype.K }, options || {});
            ...
            var onScroll = function() {
                ...
                if (top <= bodyOff.top + options.offset) { style = { position: 'fixed', top: options.offset + 'px' }; } else { style = { position: 'absolute', top: top + 'px' }; }
            ...
            element.updateTop = function(topLimit) { top = topLimit; return element; };
            Event.observe(window, 'scroll', onScroll);
            return element;
        },
        positionFixedBottom: function(element, options) {
            element = $(element);
            options = Object.extend({ offset: 0, onPinned: Prototype.K, onUnpinned: Prototype.K, onBeforeScroll: Prototype.K, onScroll: Prototype.K }, options || {});
            var off = element.cumulativeOffset();
            var sOff = element.cumulativeScrollOffset();
            var top = off.top + sOff.top;
            ...
            var onScroll = function() {
                ...
                var style = {};
                var bodyOff = $(document.body).cumulativeScrollOffset();
                if (top + h >= bodyOff.top + options.offset) { style = { position: 'fixed', bottom: options.offset + 'px' }; } else {
                    if (element.style.position == "fixed") {
                        element.setStyle({ position: 'absolute', top: bodyOff.top + options.offset + 'px' });
                        options.onBeforeScrollFail(element, parseInt(style.top, 10), bodyOff.top);
                    }
        ...
        keepInViewport: function(element, options) {
            element = $(element);
            options = Object.extend({ offset: [10, 10], offsetLeft: false, offsetTop: false, delay: 0.1, onPinned: Prototype.K, onUnpinned: Prototype.K, onBeforeScroll: Prototype.K, onScroll: Prototype.K, smooth: false, horzontal: false, vertical: true, animation: { duration: 0.2, easing: 'sineOut' }, topLimit: parseInt(element.getStyle('top') || 0, 10), leftLimit: parseInt(element.getStyle('left') || 0, 10) }, options || {});
            ...
            var onScroll = function(e) {
                ...
                if (options.smooth === false) { doScroll(); } else { timer = setTimeout(doScroll, options.delay); }
                return element;
            };
            element.pin = function() {
                options.onPinned(element);
        ...
        bigSelect: function(element, options) {
            element = $(element);
            if (!Prototype.Browser.IE9 && !Prototype.Browser.IE10 && Prototype.Browser.IE) { return element; }
            options = Object.extend({ classpreFix: 'big-select', additionalClassName: '', onSelect: function(x) { return x; }, onComplete: function(x) { return x; } }, options || {});
            if (element.selectConverted) { element.selectConverted.remove(); }
            ...
            if (options.width) { cont.setStyle({ width: options.width }); }
            ...
            element.observe('change', function() { if (options.textfield) { textfield.value = "{" + element.getSelected().value + "}"; } else { span.update(options.onSelect(element.getSelected().text)); } });
            ...
            $A(element.options).each(function(opt) {
                ...
                if (opt.hasClassName("bold")) { li.setStyle('color:#555; font-weight:bold;'); }
                li.hover(function() { li.setStyle('background:#ccc'); }, function() { li.setStyle({ background: '' }); });
            ...
            cont.observe('blur', function() { closeList(); });
            list.show();
            ...
            var toggleList = function() {
                if (list.visible()) { closeList(); } else {
                    ...
                    if (vh < lt + lh) {
                        ...
                        } else { list.setStyle({ height: (vh - lt - 20) + 'px', overflow: 'auto' }); }
                    }
                }
            };
        ...
        rotatingText: function(element, text, options) {
                element = $(element);
                options = Object.extend({ delimiter: ' - ', duration: 150 }, options || {});
...
if ("JotformFeedbackDefaultSettings" in window) { var JotformFeedbackDefaultSettings = { buttonText: "Feedback", verticalTextPadding: 7, horizontalTextPadding: 15, buttonSpace: 35, height: 500, width: 700, fontFamily: "verdana", fontColor: "#FFFFFF", fontStyle: "bold", background: "#F59202 center no-repeat", hoverBackground: "#5C5C5C", hoverFontColor: "#FFFFFF", buttonSide: "right", buttonAlign: "center", margin: 50, base: "http://www.jotform.com/", formId: null, windowTitleBgColor: false, windowTitleFontColor: false, fontSize: "18px", inlineStyle: "", iframeParameters: {}, setBorderRadius: true, type: 0, openOnLoad: false, reverseButtonText: false, reCalculate: true }; }
if ("JotformFeedbackManager" in window) {
    var JotformFeedbackManager = {
        ...
        addNewButton: function(button, reCalculate) {
            ...
            if (!Object.isArray(JotformFeedbackManager.buttons[key])) { JotformFeedbackManager.buttons[key] = []; }
        ...
        setButtons: function(key) {
            ...
            for (i = 0; i < JotformFeedbackManager.buttons[key].length; i++) {
                tempButton = JotformFeedbackManager.buttons[key][i];
                if (JotformFeedbackManager.reCalculate) { tempButton.calculateButtonPosition(total, current); } else { tempButton.calculateButtonPosition(0, 0); }
...
var JotformFeedback = Class.create({
    initialize: function(options) {
        ...
        this.ieVer = Protoplus.getIEVersion();
        ...
        if (this.options.isLink) { if (this.documanLoaded) { this.setLinkListener(); } else { document.ready(this.setLinkListener.bind(this)); } } else { if (this.documanLoaded) { this.setButtons(); } else { document.ready(this.setButtons.bind(this)); } }
        if (this.options.openOnLoad === true) { if (this.documanLoaded) { this.showWindow(); } else { document.ready(this.showWindow.bind(this)); } }
    },
    setButtons: function() {
        this.createButton();
        if (this.needGhostButton()) { this.createGhostButton(); }
    ...
    setLinkListener: function() {
        ...
        $$("." + linkIdentifier).each(function(elem) { elem.observe('click', this.showWindow.bind(this)); }.bind(this));
        if ($(linkIdentifier)) { $(linkIdentifier).observe('click', this.showWindow.bind(this)); }
    },
    needGhostButton: function() { return this.ieVer > 0 && (this.options.buttonSide === "right" || this.options.buttonSide === "left"); },
    createGhostButton: function() {
        var align;
        if (this.options.buttonSide === "right") { align = "right"; } else { align = "left"; }
        this.ghostButton = new Element("input", {
            type: "button",
            style: "position: fixed;" +
                align + ": 0px;" + "width: " + this.getButtonHeight() + "px;" + "height: " + this.getButtonWidth() + "px;" + "cursor: pointer;" + "z-index: 998;" + "border: 0;"
        });
    ...
    mouseOver: function() { this.button.setStyle({ "background": this.options.hoverBackground, "color": this.options.hoverFontColor, textShadow: "0px 0px 10px rgba(255,255,255,0.6)" }); },
    mouseOut: function() { this.button.setStyle({ "background": this.options.background, "color": this.options.fontColor, textShadow: "none" }); },
    getButtonWidth: function() { if (this.ieVer > -1 && (this.options.buttonSide === "right" || this.options.buttonSide === "left")) { return this.button.offsetHeight; } else { return this.button.offsetWidth; } },
    getButtonHeight: function() { if (this.ieVer > -1 && (this.options.buttonSide === "right" || this.options.buttonSide === "left")) { return this.button.offsetWidth; } else { return this.button.offsetHeight; } },
    createButton: function() {
        ...
        this.button = linkElement = new Element("a", { className: 'jotform-feedback-link', id: 'jotform-feedback-' + this.options.formId, style: style }).update(this.options.buttonText);
    ...
    generateBoxRadius: function() {
        ...
        switch (this.options.buttonSide) {
            ...
            case "top":
                ...
                break;
            case "right":
                if (this.options.reverseButtonText) { radiusValues = "0 0 5px 5px"; } else { radiusValues = "5px 5px 0 0"; }
                shadowValues = "";
                break;
            case "left":
                if (this.options.reverseButtonText) { radiusValues = "5px 5px 0 0"; } else { radiusValues = "0 0 5px 5px"; }
                shadowValues = "";
                break;
        }
    ...
    setTransform: function() {
        if (this.options.buttonSide === "left" || this.options.buttonSide === "right") {
            ...
            return "-moz-transform:rotate(" + degree.toString() + "deg);" + "-o-transform:rotate(" + degree.toString() + "deg);" + "-webkit-transform:rotate(" + degree.toString() + "deg);" + "filter: progid:DXImageTransform.Microsoft.BasicImage(rotation=" + ieDegree.toString() + ");" + "transform:rotate(" + degree.toString() + "deg);";
        } else { return ""; }
    },
    embedButton: function(button) { document.body.appendChild(button); },
    setButtonListeners: function(button) { button.observe('click', this.showWindow.bind(this)); },
    showButton: function() { this.button.setStyle({ visibility: "" }); },
    setOriginPointAfterRotation: function() {
        switch (this.options.buttonSide) {
            case "left":
                if (this.ieVer === -1) { this.button.setStyle({ marginLeft: -1 * (this.getButtonWidth() / 2 - this.getButtonHeight() / 2) + "px" }); }
                break;
            case "right":
                if (this.ieVer === -1) { this.button.setStyle({ marginRight: -1 * (this.getButtonWidth() / 2 - this.getButtonHeight() / 2) + "px" }); } else if (this.ieVer === 8 || this.ieVer === 9) { this.button.setStyle({ marginRight: -1 * (this.getButtonWidth() - this.getButtonHeight()) + "px" }); }
                break;
            default:
                break;
        }
    ...
    calculateButtonPosition: function(total, current) {
        ...
        switch (this.options.buttonAlign) {
            case "top":
                if (this.ieVer === -1) { this.button.setStyle({ top: this.options.buttonSpace + current + "px", marginTop: (this.getButtonWidth() / 2 - this.getButtonHeight() / 2) + "px" }); } else { this.button.setStyle({ top: this.options.buttonSpace + current + "px" }); if (this.needGhostButton()) { this.ghostButton.setStyle({ top: this.options.buttonSpace + current + "px" }); } }
                break;
            case "bottom":
                if (this.ieVer === -1) { this.button.setStyle({ bottom: this.options.buttonSpace + current + "px", marginBottom: (this.getButtonWidth() / 2 - this.getButtonHeight() / 2) + "px" }); } else if (this.ieVer === 8 || this.ieVer === 9) { this.button.setStyle({ bottom: this.options.buttonSpace + current + "px", marginBottom: (this.getButtonWidth() - this.getButtonHeight()) + "px" }); if (this.needGhostButton()) { this.ghostButton.setStyle({ bottom: this.options.buttonSpace + current + "px" }); } } else if (this.ieVer === 7) { this.button.setStyle({ bottom: this.options.buttonSpace + current + "px" }); if (this.needGhostButton()) { this.ghostButton.setStyle({ bottom: this.options.buttonSpace + current + "px" }); } }
                break;
            case "left":
            ...
            case "center":
                if (this.options.buttonSide === "left" || this.options.buttonSide === "right") {
                    var marginValue = 0;
                    if (this.ieVer === -1) { marginValue = -1 * (total / 2 - this.getButtonWidth() / 2) + -1 * this.getButtonHeight() / 2 + current; } else { marginValue = -1 * total / 2 + current; }
                    this.button.setStyle({ top: this.options.margin + "%", marginTop: marginValue + "px" });
                    if (this.needGhostButton()) { this.ghostButton.setStyle({ top: this.options.margin + "%", marginTop: marginValue + "px" }); }
                } else { this.button.setStyle({ left: this.options.margin + "%", marginLeft: (-1 * total / 2 + current) + "px" }); }
                break;
        }
    },
    showWindow: function() {
        ...
        var types = [{}, { borderWidth: 9, borderOpacity: 1, borderColor: '#fff', hideTitle: true, closeButton: '<img src="' + this.options.base + 'images/close-wiz-white.png" style="position:relative;top:-11px;right:-11px;" />' }, { borderWidth: 9, borderOpacity: 0.8, hideTitle: true, closeButton: '<img src="' + this.options.base + 'images/close-wiz-black.png" style="position:relative;top:-11px;right:-11px;" />', borderColor: '#000' }];
        Object.extend(settings, types[this.options.type]);
        Object.extend(document.windowDefaults, settings);
        this.formWindow = document.window({ title: this.options.windowTitle, width: this.options.width + "px", height: this.options.height + "px", content: "<iframe  allowtransparency=\"true\" src=\"" + this.options.base + "form/" + this.options.formId + "?" + $H(this.options.iframeParameters).toQueryString() + "\" frameborder=\"0\" style=\"width:100%;" + "height:" + this.options.height + "px; border:none;\"></iframe>" });
    },
    generateBackgroundForWindow: function() { if (this.options.windowTitleBgColor === false) { return 'url(' + this.options.base + 'images/title-bg.png)'; } else { return this.options.windowTitleBgColor; } }
});
